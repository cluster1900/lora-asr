...
- `evaluation/`、`inference/`、`notebooks/`、`scripts/` 必须各自保留 `README.md`。
- 目录 README 必须说明目录职责、当前文件清单、主要输入输出、使用入口和维护要求。
- 在上述目录新增、重命名或删除文件时，必须在同一提交同步更新对应目录的 `README.md`。
- 文件职责、CLI 参数、输入输出 schema、产物路径或恢复/门禁行为变化时，也必须同步更新对应
  `README.md`，不能只修改代码。
- `__pycache__/` 等已忽略的自动生成缓存不写入文件清单；其他人工维护文件必须列出。
- 提交前运行目录 README 合同测试，确保没有未登记文件。
